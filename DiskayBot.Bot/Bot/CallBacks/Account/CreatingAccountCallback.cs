...
public class CreatingAccountCallback : BotCommand {
    ...
    private readonly MemoryController _service;

    public CreatingAccountCallback(string callback, RedisController redis, MemoryController service) : base(callback) {
        _redis = redis;
        _service = service;
    }
    ...
    public override async Task ExecuteAsync(ITelegramBotClient bot, CancellationToken token, UserEvent evt) {
        ...
        if (callBackEvent.Query == "yes"){
                ...
                if (cash != null){
                    ...
                    var request = await _service.CreateUser(evt.UserId, evt.Username, groupId.ToString());
                    if (request) {
